...
def _record_movement_data(movement_data_writer, movement):
    ...
    total_time = int((exit_time - entry_time).total_seconds())
    
    # Flatten and convert positions to a list
    positions = np.array(movement.positions).flatten().tolist()

    # Prepare data for CSV row, including total time
    data = [track_id, entry_time, exit_time, total_time] + positions
